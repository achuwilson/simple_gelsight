...
def get_image():
    stream=urllib.request.urlopen('http://raspberrypi.local:8088/stream')
    bytess=b''
    while True:
...
while True:
    ...
    alpha =0.8
    beta = 1.25
    pfilar = np.where((points[:,2]>(np.max(points[:,2])*alpha)))
    fil2points=points[pfilar]
    obj2=np.where((fil2points[:,2])>(avg_z*beta))
    filpoints = fil2points[obj2]
    ...
    if count<=10:
        ...
        avg_arr.append(np.average(points[:,2]))
    if count==10:
        avg_z=np.average(avg_arr)
